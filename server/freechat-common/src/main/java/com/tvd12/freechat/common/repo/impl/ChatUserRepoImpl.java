...
public class ChatUserRepoImpl
	...
	@Override
	public List<ChatUser> findByUsername(String keyword, String owner, int skip, int limit) {
		Query<ChatUser> query = newQuery();
		query.and(
				query.criteria("username").contains(keyword),
				query.criteria("username").notEqual(owner)
		);
